...
class Localization:
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        # TODO
        # print(msg)
        pos = Localization.spherical_to_cartesian(np.array([msg.latitude, msg.longitude]), np.array([42.293195, -83.7096706]))
        self.pose = SE3(pos, self.pose.rotation)
    ...
    def imu_callback(self, msg: Imu):
        ...
        # TODO
        # print(msg)
        self.pose = SE3.from_pos_quat(self.pose.position, np.array([msg.orientation.x,
                                                                    msg.orientation.y,
                                                                    msg.orientation.z,
                                                                    msg.orientation.w]))
        self.pose.publish_to_tf_tree(self.tf_broadcaster, "map", "base_link")
    ...
    def spherical_to_cartesian(spherical_coord: np.ndarray, reference_coord: np.ndarray) -> np.ndarray:
        ...
        # TODO
        R = 6371000
        # x = R * np.radians(spherical_coord[1] - reference_coord[1]) * np.cos(np.radians(reference_coord[0]))
        x = (R * (np.radians(spherical_coord[1]) - np.radians(reference_coord[1])) * np.cos(np.radians(reference_coord[0])))
        # y = R * np.radians(spherical_coord[0] - reference_coord[0])
