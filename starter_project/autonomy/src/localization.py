...
class Localization:
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        """
        refPoint = np.array([(42.293195),(-83.7096706)])
        sph = np.array([(msg.latitude), (msg.longitude)])
        cartesian = self.spherical_to_cartesian(sph,refPoint)
        self.pose = SE3(cartesian.copy(), self.pose.rotation)
    ...
    def imu_callback(self, msg: Imu):
        ...
        self.pose = SE3.from_pos_quat(self.pose.position, np.array([orientx,orienty,orientz,orientw]))
        self.pose.publish_to_tf_tree(self.tf_broadcaster, "map", "base_link")
    ...
    def spherical_to_cartesian(spherical_coord: np.ndarray, reference_coord: np.ndarray) -> np.ndarray:
        ...
        R = 6371000
        cartesian[0] = (R* (np.radians(spherical_coord[1]) - np.radians(reference_coord[1])) * np.cos(np.radians(reference_coord[0])))
        cartesian [1]= R * (np.radians(spherical_coord[0]) - np.radians(reference_coord[0]))
        return cartesian
