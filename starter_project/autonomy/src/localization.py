...
class Localization:
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        cartesian = self.spherical_to_cartesian(spherical, reference)
    ...
    def spherical_to_cartesian(spherical_coord: np.ndarray, reference_coord: np.ndarray) -> np.ndarray:
        ...
        latitude = np.radians(spherical_coord[0]) - np.radians(reference_coord[0])
        longitude = np.radians(spherical_coord[1]) - np.radians(reference_coord[1])

        y = circumference * latitude 

        x = circumference * longitude * np.cos(np.radians(reference_coord[0]))
