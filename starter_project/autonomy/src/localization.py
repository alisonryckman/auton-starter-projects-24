...
class Localization:
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        spherical_coordinate = np.array([(msg.latitude),(msg.longitude)])
        reference_coordinate = np.array([(42.293195),(-83.7096706)])

        position = self.spherical_to_cartesian(spherical_coordinate,reference_coordinate)

        self.pose = SE3.from_pos_quat(position,self.pose.rotation.quaternion) # was also . quaternian
    ...
    def imu_callback(self, msg: Imu):
        ...
        self.pose = SE3.from_pos_quat(self.pose.position,orientation)
    ...
    def spherical_to_cartesian(spherical_coord: np.ndarray, reference_coord: np.ndarray) -> np.ndarray:
        ...
        x = (6371000.0 * (np.radians(spherical_coord[1]) - np.radians(reference_coord[1])) * np.cos(np.radians(reference_coord[0])))
        y = 6371000.0 * (np.radians(spherical_coord[0]) - np.radians(reference_coord[0]))
        z = 0.0
