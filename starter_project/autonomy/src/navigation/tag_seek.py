...
class TagSeekState(BaseState):
    ...
    def evaluate(self, ud):
        ...
        tag = self.context.env.get_fid_data(self)
        ...
        if tag.closenessMetric < DISTANCE_TOLERANCE and abs(tag.xTagCenterpixel) >= ANGULAR_TOLERANCE:
            return "success"
        ...
        if tag.closenessMetric >= DISTANCE_TOLERANCE:
            twist.linear.x = 1
        if abs(tag.xTagCenterPixel) >= ANGULAR_TOLERANCE:
